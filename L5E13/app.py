...
def infer_on_video(args, model):
    ...
    mqttc = mqtt.Client()
    mqttc.connect(HOSTNAME, port=MQTT_PORT, keepalive=MQTT_KEEPALIVE_INTERVAL, bind_address=IPADDRESS)
    ...
    while cap.isOpened():
        ...
        if plugin.wait() == 0:
            ...
            #mqttc.publish("class_names", speed)
        ...
        '''a = ffmpeg._init()
        a.__loader__(out_frame)'''
        sys.stdout.buffer.write(out_frame.tostring())
        #sys.stdout.flush()
        #print("OUT_FRAME", out_frame)
...
#python app.py -i test_video.mp4 | ffmpeg -video_size 1280x720 -i - http://0.0.0.0:3004/fac.ffm